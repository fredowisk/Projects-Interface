        ...
        <TeamList>
          {teams.data.map((team) => (
            <Team key={team.id} onClick={() => this.handleTeamSelect(team)}>
              <img
                src={`https://ui-avatars.com/api/?bold=true&font-size=0.33&background=7159c1&color=fff&name=${team.name}`}
                alt={team.name}
              />
            </Team>
          ))}
            ...
            <Modal>
              ...
              <form onSubmit={this.handleCreateTeam}>
                ...
                <div>
                  ...
                  <Button onClick={closeTeamModal} size="default" type="button" color="gray">
                    Cancelar
                  </Button>
                </div>
              </form>
